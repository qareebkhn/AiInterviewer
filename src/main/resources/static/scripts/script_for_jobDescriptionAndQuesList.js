document.addEventListener('DOMContentLoaded', function() {
	...
	document
		...
		.addEventListener(
			...
			function() {
				...
				document.getElementById('usernameProfile').value = 'JohnDoe';
				document.getElementById('emailProfile').value = 'johndoe@example.com';
	...
	const textareaQues = document.getElementById('job-description');
	const charCountSpan = document.getElementById('char-count');
	const maxChars = 5000;

	function updateCharCount() {
		const currentLength = textareaQues.value.length;
		if (currentLength > maxChars) {
			textareaQues.value = textareaQues.value.substring(0, maxChars); // Trim excess characters
		}
		charCountSpan.textContent = `${maxChars - textareaQues.value.length} chars left`;
	}

	textareaQues.addEventListener('input', updateCharCount);
	updateCharCount(); // Initialize char count on page load

	/*// Another textarea char count functionality
	...
	updateCharCountAnswer(); // Initialize char count on page load*/
...
function submitAnswerForm() {
	...
	var textareaData = document.getElementById('answerText').value;

	// Set the value of the hidden input field
	document.getElementById('answer').value = textareaData;
